...
- **便携模式（绿色 / U 盘无痕）**：**设置 → 数据 → 便携模式** 勾选即可开启 —— 它在 `QuickText.exe` 旁建一个标记文件 `QuickText.portable`，**重启后生效**（首次以便携方式启动时，会把你的设置和使用记录一并带过去，不会从头配置）。之后设置、使用次数、每日备份和默认文本库都改放到 **`<exe 所在目录>\Data\`**（不再写 `%APPDATA%` / 我的文档），「开机自启」也改用**启动文件夹快捷方式**而非注册表 —— 整套工具随 U 盘走，宿主机上零残留。需把程序放在可写位置（U 盘等，非 `Program Files`）；**文本库用「导出 / 导入备份」搬到新位置**。开机自启按模式各自记录，切换模式后如要自启请在新模式下重新勾一次。不开启则用安装式布局（数据夹是同步盘时应选这种）。<br>_（这个开关必须靠标记文件实现，不能只是个普通设置项——因为它决定了 `settings.json` 本身存在哪；切换只在下次启动生效，不会扰动当前会话。）_
