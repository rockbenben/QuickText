...
> **Hassas bir şey saklamadan önce.** Parçacıklar **düz metin JSON** olarak kaydedilir; bilerek şifrelenmez — dosyayı kendiniz okuyabilin, karşılaştırabilin ve düzenleyebilin diye. Yalnızca sizin kullandığınız bir makinede şablonlar ve kalıp metinler için bu gayet uygundur. Ama şu anlama da gelir: oraya koyduğunuz kimlik numarası ya da parola, o dosyayı okuyabilen her şey tarafından okunabilir — hesabınızla çalışan diğer programlar dâhil, klasörü Dropbox / OneDrive'a koyduysanız senkronizasyon sağlayıcısı dâhil. Gerçek sırlar için bir parola yöneticisi kullanın; QuickText'e ise ortada durmasından rahatsız olmayacağınız metni bırakın.
...
> **Uzun metin / kod**: gövdenin üzerindeki `⤢` düğmesi, onu neredeyse tam ekran bir editörde açar (veya gövdede `Ctrl+Shift+Enter` tuşlarına basın); `Esc` veya "Bitti" düzenlemeyi bitirir; bir şey değiştirdiyseniz önce kaydetmek mi yoksa atmak mı istediğiniz sorulur. "{variable} yer tutucularını etkinleştir" işaretliyken, tokenlar **türe göre renklendirilir** — değişkenler camgöbeği, `{date}` gibi otomatik tokenlar amber, `{snippet:x}` mor, `{cursor}` camgöbeği kesikli — yanlış yazılmış bir snippet adı, geçersiz bir tarih biçimi veya kapanmamış bir süslü parantez ise, üzerine gelindiğinde nedenini gösteren **kırmızı dalgalı bir çizgi** alır. Bunların hepsi gönderim sırasında **olduğu gibi** yapıştırılır; bu daha önce ancak yanlış bir şey yapıştırdıktan sonra fark edilebiliyordu. Bir tarih tokenının üzerine gelmek, çözümlenmiş değerinin önizlemesini de gösterir. Kod için kutuyu işaretsiz bırakın: vurgulama tamamen devre dışı kalır, böylece `{}` dolu bir gövde sessiz kalır ve durum çubuğu yalnızca kaç tokenın olduğu gibi çıkacağını belirtir. Ayrıca eklendi: **Enter girintiyi korur** ve **`Tab` çok satırlı bir seçimin tamamına girinti uygular**.
>
> Büyütülmüş pencere **her zaman satır numaralarını gösterir** ve alt kısmında bir **kod biçimi** seçici sunar — JSON, YAML, XML, HTML, Markdown, SQL, Python, JavaScript/TypeScript, C#, Java, PowerShell, Shell ve INI, toplam 13 — bu, gövdenin sözdizimini vurgular ve snippet başına hatırlanır. **Saklanan metin düz kalır ve yapıştırılan şey değişmez.** Yer tutucu vurgulaması arka planı boyar, sözdizimi vurgulaması ise karakterleri renklendirir, böylece ikisi asla çakışmaz: içinde `{variables}` bulunan bir JSON gövdesi hem yapısını hem de değişkenlerini aynı anda gösterir.
...
| `{date}` `{time}` `{datetime}` | Geçerli tarihi/saati ekler; `{date+7}` (7 gün sonrası) gibi kaymaları ve `{date=yyyy-MM-dd}` / `{time=HH:mm:ss}` gibi özel biçimleri destekler; kaymalarla birleştirilebilir: `{date+7=MM-dd}`. Çince takma adlar `{日期}` / `{时间}` / `{日期时间}` de çalışır |
...
- **İçerik**: düz metin (çok satırlı, özel karakterler, kayıpsız emoji), yer tutucular (varsayılanlar / seçenek açılır listeleri / snippet iç içe geçirme / özel tarih biçimleri / uuid / random — **snippet başına isteğe bağlı**), **görseller** (panodan veya dosyadan, gönderimde görsel olarak yapıştırılır; **görsellerin de kısaltması olabilir** — kısaltmayı yazın, görseli alın).
