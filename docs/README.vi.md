...
| `{date}` `{time}` `{datetime}` | Chèn ngày/giờ hiện tại; hỗ trợ độ lệch như `{date+7}` (7 ngày sau) và định dạng tùy chỉnh như `{date=yyyy-MM-dd}` / `{time=HH:mm:ss}`, có thể kết hợp với độ lệch: `{date+7=MM-dd}`. Các bí danh tiếng Trung `{日期}` / `{时间}` / `{日期时间}` cũng hoạt động |
