...
> Ngoài ra: `date` / `time` / `datetime` / `now` / `日期` / `时间` / `日期时间` giờ là **tên ngày giờ dành riêng** — `{date:xyz}` được đọc là định dạng ngày, không còn là "biến tên date với giá trị mặc định xyz"; hãy đổi tên các biến từng dùng những tên này.
