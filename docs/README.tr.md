...
> Ayrıca: `date` / `time` / `datetime` / `now` / `日期` / `时间` / `日期时间` artık **ayrılmış tarih adlarıdır** — `{date:xyz}` bir tarih biçimi olarak okunur, artık "varsayılanı xyz olan date adlı bir değişken" değildir; bu adları kullanan değişkenleri yeniden adlandırın.
